...
export function Provenance({ kind, children }: { kind: ProvenanceKind; children?: string }) {
  ...
  return <span data-testid={`badge-provenance-${kind}`} className="inline-flex items-center gap-1.5 rounded-md border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-2 py-1 text-[11px] font-semibold text-[hsl(var(--muted-foreground))]"><Icon size={12} />{children ?? labels[kind]}</span>;
}
...
export function MiniBar({ values, color = "hsl(var(--primary))" }: { values: number[]; color?: string }) {
  ...
  return <div className="flex h-16 items-end gap-1.5" aria-label="trend chart">{values.map((value, index) => <div key={index} className="flex-1 rounded-t-sm opacity-90" style={{ height: `${Math.max(12, value / max * 100)}%`, backgroundColor: color }} />)}</div>;
}
