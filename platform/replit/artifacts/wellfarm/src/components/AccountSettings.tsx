...
export function AccountSettings() {
  ...
  return <section className="mt-7 space-y-6">
    ...
    <section className="rounded-xl border border-destructive/30 p-6"><h2 className="text-xl font-bold">Delete account</h2><p className="mt-2 text-sm text-muted-foreground">Permanently remove your Wellfarm account, profile photo, scans, reports, sessions and feedback.</p>{shared && <p className="mt-2 text-sm text-muted-foreground">Your shared Indra sign-in identity is kept. This deletes only your Wellfarm data, not your Indra account.</p>}<button onClick={() => setOpen(true)} className="mt-4 rounded-lg border border-destructive px-4 py-3 text-destructive">Delete my account</button></section>
    <Dialog open={open} onOpenChange={value => { if (!busy) {setOpen(value); setPassword(""); setError("");} }}><DialogContent><DialogHeader><DialogTitle>Delete your Wellfarm account?</DialogTitle><DialogDescription>This cannot be undone. Enter your account email (Supabase) or current password (local accounts) to permanently delete your account and its saved data. {shared && "Your shared Indra login will not be deleted."}</DialogDescription></DialogHeader><form onSubmit={async event => {
