...
export function AccountSettings() {
  ...
  return <section className="mt-7 space-y-6">
    ...
    <section className="rounded-xl border border-destructive/30 p-6"><h2 className="text-xl font-bold">Delete account</h2><p className="mt-2 text-sm text-muted-foreground">Permanently remove your account, profile photo, scans, reports, sessions and feedback.</p><button onClick={() => setOpen(true)} className="mt-4 rounded-lg border border-destructive px-4 py-3 text-destructive">Delete my account</button></section>
    <Dialog open={open} onOpenChange={value => { if (!busy) {setOpen(value); setPassword(""); setError("");} }}><DialogContent><DialogHeader><DialogTitle>Delete your Wellfarm account?</DialogTitle><DialogDescription>This cannot be undone. Enter your password to permanently delete your account and its saved data.</DialogDescription></DialogHeader><form onSubmit={async event => {
      event.preventDefault(); setBusy(true); setError("");
      try { const response = await fetch("/api/account", {method: "DELETE", headers: {"Content-Type": "application/json"}, body: JSON.stringify({password})}); if (!response.ok) throw new Error((await response.json()).error?.message ?? "Deletion failed."); window.location.assign("/login"); }
      catch (error) {setError(error instanceof Error ? error.message : "Connection failed.");} finally {setBusy(false);}
    }}><label className="text-sm">Current password<input required type="password" autoComplete="current-password" maxLength={128} value={password} onChange={event => setPassword(event.target.value)} className="mt-2 w-full rounded-lg border p-3" /></label>{error && <p role="alert" className="mt-3 text-destructive">{error}</p>}<div className="mt-5 flex justify-end gap-3"><button type="button" disabled={busy} onClick={() => setOpen(false)} className="rounded-lg border px-4 py-2">Cancel</button><button disabled={busy} className="rounded-lg bg-destructive px-4 py-2 text-destructive-foreground">{busy ? "Deleting…" : "Delete permanently"}</button></div></form></DialogContent></Dialog>
  </section>;
}
