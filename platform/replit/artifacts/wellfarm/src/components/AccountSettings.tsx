...
export function AccountSettings() {
  ...
  useEffect(() => {fetch("/api/auth/config").then(r => r.json()).then(config => setProvider(config.provider)).catch(() => {});}, []);
  ...
  return <section className="mt-7 space-y-6">
    ...
    <Dialog open={open} onOpenChange={value => { if (!busy) {setOpen(value); setPassword(""); setError("");} }}><DialogContent><DialogHeader><DialogTitle>Delete your Wellfarm account?</DialogTitle><DialogDescription>This cannot be undone. Enter your account email (Supabase) or current password (local accounts) to permanently delete your account and its saved data.</DialogDescription></DialogHeader><form onSubmit={async event => {
