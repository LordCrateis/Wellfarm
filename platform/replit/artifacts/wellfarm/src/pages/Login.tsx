...
function PasswordField({label,value,onChange,autoComplete,minLength}:{label:string;value:string;onChange:(value:string)=>void;autoComplete:string;minLength?:number}) {
  ...
  return <label className="block text-sm">{label}<span className="relative mt-2 block"><input required type={visible?"text":"password"} autoComplete={autoComplete} minLength={minLength} maxLength={128} value={value} onChange={event=>onChange(event.target.value)} className="w-full rounded-lg border bg-background p-3 pr-12"/><button type="button" onClick={()=>setVisible(show=>!show)} aria-label={visible?`Hide ${label.toLowerCase()}`:`Show ${label.toLowerCase()}`} aria-pressed={visible} className="absolute inset-y-0 right-0 flex w-12 items-center justify-center rounded-r-lg text-muted-foreground transition-colors hover:bg-secondary hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset">{visible?<EyeOff size={18}/>:<Eye size={18}/>}</button></span></label>;
}
export function Login() {
  ...
  return <main className="h-[100dvh] overflow-hidden bg-background px-5 py-[clamp(1.25rem,3vh,2.5rem)]"><div className="mx-auto flex h-full max-w-6xl flex-col"><div className="shrink-0"><Brand /></div>
    <div className="grid min-h-0 flex-1 items-center gap-12 lg:grid-cols-2 lg:gap-24">
      ...
      <section className="mx-auto w-full max-w-xl rounded-2xl border bg-card p-[clamp(1.25rem,3vh,2.25rem)] shadow-sm">{(register || verification) && <ol aria-label="Account setup progress" className="mb-5 grid grid-cols-3 gap-2 text-xs font-semibold"><li className="text-primary"><span className="mr-1 inline-grid h-6 w-6 place-items-center rounded-full bg-primary text-primary-foreground">1</span>Account</li><li className={verification ? "text-primary" : "text-muted-foreground"}><span className={`mr-1 inline-grid h-6 w-6 place-items-center rounded-full ${verification ? "bg-primary text-primary-foreground" : "border"}`}>2</span>Verify</li><li className="text-muted-foreground"><span className="mr-1 inline-grid h-6 w-6 place-items-center rounded-full border">3</span>Profile</li></ol>}<h2 className="text-3xl">{verification ? "Check your inbox" : register ? "Create your account" : "Welcome back"}</h2><p className="mt-2 text-sm text-muted-foreground">{verification ? `Enter the verification code sent to ${email}.` : register ? "First create your secure account. We’ll ask about you and your farm after email verification." : "Sign in to pick up where you left off."}</p>
      {!verification && <><button type="button" disabled={!config?.googleEnabled || busy} onClick={() => window.location.assign("/api/auth/google")} className="mt-5 flex min-h-12 w-full items-center justify-center gap-3 rounded-lg border bg-background p-3 font-semibold transition-colors hover:border-primary/50 hover:bg-secondary disabled:opacity-50"><GoogleMark/>Continue with Google</button>{!config?.googleEnabled && <p className="mt-2 text-xs text-muted-foreground">Google sign-in setup pending.</p>}<div className="my-4 flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border" />or use email<span className="h-px flex-1 bg-border" /></div></>}
      ...
      {verification ? <><label className="block text-sm">Email verification code<input required inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6,10}" maxLength={10} value={code} onChange={event => setCode(event.target.value.replace(/\D/g,""))} className={inputClass} /></label>{resendStatus && <p role="status" className="text-sm text-muted-foreground">{resendStatus}</p>}</> : <>
        ...
        <PasswordField label="Password" value={password} onChange={setPassword} autoComplete={register?"new-password":"current-password"} minLength={register?12:undefined}/>
        {register && <><PasswordField label="Confirm password" value={confirmation} onChange={setConfirmation} autoComplete="new-password" minLength={12}/><p className="text-xs text-muted-foreground">Use at least 12 characters and a password you don’t use elsewhere.</p></>}
