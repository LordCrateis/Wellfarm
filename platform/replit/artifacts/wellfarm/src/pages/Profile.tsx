      ...
      <form onSubmit={async event => { event.preventDefault(); if (!await saveProfile(draft)) { setFeedback("error"); return; } setLocale(draftLocale); setDraft({ ...draft, name: draft.name.trim(), farm: draft.farm.trim() }); setFeedback("saved"); }} onChange={() => setFeedback(null)} className="min-w-0 space-y-6">
        <section className="rounded-xl border bg-card p-5 md:p-7"><h2 className="text-lg font-bold">About you and your farm</h2><p className="mt-1 text-sm text-muted-foreground">Only add the details you want to use in your workspace.</p>
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <label className="text-sm font-semibold">First name<input autoComplete="given-name" maxLength={50} value={draft.firstName ?? ""} onChange={e => setDraft({ ...draft, firstName: e.target.value, name: `${e.target.value} ${draft.lastName ?? ""}`.trim() })} className={fieldClass} /></label>
            <label className="text-sm font-semibold">Last name<input autoComplete="family-name" maxLength={50} value={draft.lastName ?? ""} onChange={e => setDraft({ ...draft, lastName: e.target.value, name: `${draft.firstName ?? ""} ${e.target.value}`.trim() })} className={fieldClass} /></label>
            ...
            <label className="text-sm font-semibold">City or village<input autoComplete="address-level2" maxLength={100} value={draft.city ?? ""} onChange={e => setDraft({...draft,city:e.target.value})} className={fieldClass} /></label>
